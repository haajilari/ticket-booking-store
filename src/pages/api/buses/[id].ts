// src/pages/api/buses/[id].ts
import type { NextApiRequest, NextApiResponse } from "next";
import { TicketType, BusTicket } from "@/types/ticket";

// Mock data for buses (should be consistent with data in index.ts in the same directory or sourced from a common module)
...
/**
 * @description API route for retrieving information about a specific bus based on ID.
...
export default function handler(
  ...
  const { id } = req.query; // id is extracted from the URL path (e.g., /api/buses/f123)

  if (req.method === "GET") {
    const bus = mockBuses.find((f) => f.id === id);
